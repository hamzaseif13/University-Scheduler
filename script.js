class Database {
  ...
  search(val, searchBy = "lineNumber") {
...
class Scheduler {
  ...
  get courses() {
    // getter to access #myCourses from outside the class
  ...
  _addCourseFunction(courseNum) {
    if (this.courseIndex(courseNum) != -1)
...
class SchedulerGUI {
  ...
  #getElements() {
    ...
    this.#table.tableObj.table = table.querySelector(".timeTable");;
  ...
  updateModal(
    ...
    for (const course of arr) {
      this.#myModal.body.appendChild(
        this.#generateHTMLCourseCard(course, highlight, prop)
      );
    }
  }
